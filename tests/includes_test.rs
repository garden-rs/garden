...
fn read_includes() -> Result<()> {
    ...
    let actual = garden::eval::value(&app_context, config, "${var_included}");
...
fn template_includes() -> Result<()> {
    let app_context =
        garden::model::ApplicationContext::from_path_string("tests/data/garden.yaml")?;
    let config_id = app_context.get_root_id();
    let context = garden::query::find_tree(&app_context, config_id, "tree-echo", None)?;
    let config = app_context.get_root_config();
    ...
    let context = garden::query::find_tree(&app_context, config_id, "tree-echo-extended", None)?;
    let tree = config
        .trees
        .get(&context.tree)
    ...
    let context = garden::query::find_tree(&app_context, config_id, "tree-echo-nested", None)?;
    let tree = &config
        .trees
        .get(&context.tree)
    ...
    let context = garden::query::find_tree(
        &app_context,
        config_id,
    ...
    let context =
        garden::query::find_tree(&app_context, config_id, "tree-echo-extended-tree", None)?;
    let result = garden::eval::tree_value(
        &app_context,
        config,
        "${template-variable}",
    ...
    let context = garden::query::find_tree(&app_context, config_id, "replacement-tree", None)?;
    let result = garden::eval::tree_value(
        &app_context,
        config,
        "${tree-variable}",
